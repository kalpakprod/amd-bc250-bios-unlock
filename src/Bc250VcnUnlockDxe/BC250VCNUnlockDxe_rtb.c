  ...
  )
{
  ...
  (VOID)Context;
  gBS->CloseEvent (Event);  /* one-shot */

  DEBUG ((DEBUG_INFO, "Bc250VCNUnlock: ReadyToBoot, running sequence\n"));
  Result = VcnEnableSequence ();
  if (EFI_ERROR (Result)) {
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "Bc250VCNUnlock: entry (v009-rtb, registering)\n"));
  Result = EfiCreateEventReadyToBootEx (
             TPL_CALLBACK,
             VcnReadyToBootCallback,
             NULL,
             &Event
  ...
  if (EFI_ERROR (Result)) {
    DEBUG ((DEBUG_ERROR, "VCN: ReadyToBoot register failed %r\n", Result));
    return Result;
  }
