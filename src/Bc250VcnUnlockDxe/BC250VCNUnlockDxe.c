/** @file BC250-VCN-Unlock DXE driver — v003b (route B: direct replay).
...
VcnEnableSequence (VOID)
{
  ...
  Bitmap = SmnRead32 (SMN_DOMAIN_BITMAP);
  ...
  /* step 0b: log the plan + the live controller pre-state */
  for (i = 0; i < SMN_VCN_DESC_WORDS; i++) {
    DEBUG ((DEBUG_INFO, "VCN: plan A/B[%u] = 0x%08x / 0x%08x\n",
            i,
            SmnRead32 (SMN_CLOCK_PLAN_A + SMN_VCN_DESC_OFF + i * 4U),
            SmnRead32 (SMN_CLOCK_PLAN_B + SMN_VCN_DESC_OFF + i * 4U)));
  }
