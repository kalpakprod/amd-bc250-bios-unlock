...
<p align="center">
  <img src="docs/assets/hero.svg" width="100%" alt="Разблокировка BIOS AMD BC-250: блок кодирования VCN, включённый в прошивке. Панель доказательств: заголовок LZMA исправлен с неизвестного размера на явный, лесенка v005 грузит DXE, v006r на чипе.">
</p>
...
- Оставшийся клин DXE разложен на лесенку из 5 ступеней, по одной переменной за раз.
...
Как читать имя версии:

- `vNN` — порядковый номер эксперимента (v004 был раньше v005).
- `r` — пересборка: тот же эксперимент, исправленные байты (v006r заменяет v006).
- Слово — вариант кода: `noop` ничего не делает, `active` выполняет полную
  последовательность, `secure` только безопасно читает, `rtb` откладывается до ReadyToBoot.
- `ownfv-insert` и `append` — размещение внутри тома прошивки.
...
Ступени (образы и манифесты — в [Releases](https://github.com/kalpakprod/amd-bc250-bios-unlock/releases)):

- **v004** (`98b9c2bd…`), образец LZMA-бага: несёт route-B-драйвер с
  заголовком LZMA неизвестного размера. Доказывает класс зависания.
  Вердикт: прошит, встаёт после 1 моргания.
- **v005** (`2fadb173…`), загрузчик DXE: v004 плюс 8-байтовый фикс LZMA.
  Доказывает, что плата снова грузит DXE. Вердикт: прошит, встаёт после
  2 морганий, без LAN и видео.
- **v006r-noop** (`6500bea5…`), контрольная заглушка: драйвер грузится, но
  выполняет ноль аппаратных операций. Отделяет вину драйвера от вины
  хирургии тома. Вердикт: на чипе, ждём признаков холодной загрузки.
- **v007-active** (`9d249397…`), тест позиции: полный драйвер дописан в конец
  тома вместо ранней вставки. Проверяет позицию диспетча. Вердикт: на полке,
  шьём если v006r загрузится.
- **v007r-noop** (`0133a34b…`), дописанный контроль: заглушка дописана в конец
  тома. Различает вставку и дописывание. Вердикт: на полке, шьём если v006r встанет.
- **v008-secure** (`c935e3ec…`), безопасный читатель: битмап через безопасное
  чтение Q3, сырые отладочные чтения удалены. Проверяет, были ли клином сырые
  чтения. Вердикт: на полке, шьём если v006r загрузится, а v007 встанет.
- **v009-rtb** (`56df6244…`), отложенный запуск: последовательность срабатывает
  по ReadyToBoot, после подключения всех драйверов. Проверяет время диспетча.
  Вердикт: на полке, шьём если v006r загрузится, а v007+v008 встанут.

## Воспроизвести за 5 минут (без железа)

- Скачай вложения v1.0.0 (как минимум образ v004).
- Пересобери v005 и v006r байт-в-байт:
...
- Посмотри, как v004 ВАЛИТ гейт LZMA префлайта, а v005 проходит:
...
- Со своим дампом 16 МиБ (`BC250_BASE`, `BC250_PRE_SHA`) сборщики дописывания
  (v007–v009) работают против ТВОЕЙ базы.
...
- **Зависание на LZMA неизвестного размера.** Наш перепаковщик капсулы (Python
  ...
  Исправлено 8 байтами в v005; вся история: [docs/05-lzma-lesson.md](docs/05-lzma-lesson.md).
...
- **Позиция диспетча.** Наш первый слот был ПЕРВЫМ драйвером TRUE-пула, раньше
  ...
  отсутствия DEPEX (спецификация PI); одинаковые зависания v003e/v004 это
...
- **Клином DXE.** v005 встаёт до видео и LAN. Анализ набора записей против
